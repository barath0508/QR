...
export default function QRStudio({ 
  ...
  const isLogoZone = (r, c, size, hasLogo) => {
  ...
  const drawCornerEye = (ctx, x, y, cellSize, outerColor, innerColor, style) => {
  ...
  const renderCanvas = async () => {
    ...
    try {
      const textToEncode = currentEncodedText;
      ...
      const effectiveOuterEye = useSeparateEyeColors ? eyeOuterColor : fgColor;
      const effectiveInnerEye = useSeparateEyeColors ? eyeInnerColor : fgColor;
      ...
      drawCornerEye(ctx, marginCells * cellSize, marginCells * cellSize, cellSize, effectiveOuterEye, effectiveInnerEye, eyeStyle);
      drawCornerEye(ctx, (size - 7 + marginCells) * cellSize, marginCells * cellSize, cellSize, effectiveOuterEye, effectiveInnerEye, eyeStyle);
      drawCornerEye(ctx, marginCells * cellSize, (size - 7 + marginCells) * cellSize, cellSize, effectiveOuterEye, effectiveInnerEye, eyeStyle);
      ...
      console.error('Error rendering QR canvas:', err);
    }
  };
